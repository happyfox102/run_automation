...
class Config:
    def __init__(self):
        ...
        self.verify_input = False  # Отключаем проверку ввода, так как нет OCR
        self.max_attempts = 3  # Уменьшаем количество попыток, так как нет OCR проверок
...
class FormAction:
    ...
    def clear_field(self, speed_factor: float = 1.0) -> bool:
        """Очистить поле ввода без OCR"""
    ...
    def execute(self, speed_factor: float = 1.0, use_image: bool = False, verify: bool = False,
                max_attempts: int = 3, use_clipboard: bool = True) -> bool:
        """Выполнить действие без OCR проверок"""
        for attempt in range(max_attempts):
            try:
                ...
                # Очистка поля
                if not self.clear_field(speed_factor):
                ...
                # Если проверка отключена (verify=False), просто продолжаем
                time.sleep(self.delay_after * speed_factor)
                logging.info(f"Поле '{self.field.name}' заполнено значением '{self.value}'")
                return True

            except Exception as e:
                logging.error(f"Ошибка при заполнении поля '{self.field.name}': {e}")
                if attempt < max_attempts - 1:
                    time.sleep(1.0 * speed_factor)  # Пауза перед повтором
                    continue
...
class ExcelProcessor:
    ...
    def load_excel(filepath: str) -> Optional[pd.DataFrame]:
        try:
            ...
            df = df.fillna('')
            # Исправление устаревшего метода applymap
        ...
        try:
            ...
            for field in self.form_manager.fields:
                ...
                success = action.execute(
                    ...
                    verify=self.config.verify_input,  # Всегда False, так как отключили OCR
...
class SimpleGUI:
    ...
    def setup_ui(self):
        ...
        # Убираем чекбокс "Проверять ввод", так как OCR отключен
    ...
    def start_automation(self):
        ...
        self.config.verify_input = self.verify_input_var.get()  # Всегда будет False
...
def main():
    ...
    try:
        ...
        print("pip install pandas openpyxl pyautogui keyboard python-dateutil pillow pyperclip")
